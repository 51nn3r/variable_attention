...
class VariableAttention(Layer):
    def __init__(
        ...
        self._conv_time_steps_dense = None
    ...
    def _build_from_signature(self, query, value=None, key=None):
        ...
        with tf_utils.maybe_init_scope(self):
            ...
            if self._input_steps != self._output_steps:
                self._conv_time_steps_dense = self._make_conv_time_steps_dense(
                    free_dims,
                    self._get_common_kwargs_for_sublayer(),
                    "conv_time_steps"
                )
    ...
    def _make_conv_time_steps_dense(self, free_dims, common_kwargs, name=None):
        """Builds the output projection matrix.
        ...
        Returns:
          Projection layer.
        """
        output_shape = [self._output_depth, self._output_steps]
        einsum_equation, bias_axes, output_rank = _build_proj_equation(
            free_dims, bound_dims=1, output_dims=len(output_shape) - 1, common_axes=1, reverse=True
        )
        return core.EinsumDense(
            einsum_equation,
            output_shape=_get_output_shape(output_rank - 1, output_shape),
    ...
    def call(
        ...
        if self._conv_time_steps_dense:
            attention_output = self._conv_time_steps_dense(attention_output)

            rank = attention_output.shape.rank
            attention_output = tf.transpose(
                attention_output,
                [i for i in range(rank - 2)] + [rank - 1, rank - 2]
            )
