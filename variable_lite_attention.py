...
def _get_transpose_axes(dims_count):
...
def _build_proj_equation(free_dims, bound_dims, output_dims, common_axes=0, reverse=False):
    ...
    if not reverse:
        for i in range(common_axes):
            char = _CHR_IDX[i + letter_offset]
            input_str += char
            kernel_str += char
            output_str += char

        letter_offset += common_axes
...
class VariableLiteAttention(Layer):
    def __init__(
            self,
            num_heads,
            key_dim,
        ...
        self._conv_time_steps_dense1, self._conv_time_steps_dense2 = None, None
    ...
    def _build_from_signature(self, query, value=None, key=None):
        ...
        if value is None:
            self._value_shape = self._query_shape
        ...
        self._input_steps = self._query_shape[-3]

        if self._output_types is None:
            self._output_types = self._query_shape[-2]

        if self._output_steps is None:
            self._output_steps = self._input_steps

        if self._output_depth is None:
            self._output_depth = self._query_shape[-1]

        # Any setup work performed only once should happen in an `init_scope`
        # to avoid creating symbolic Tensors that will later pollute any eager
        # operations.
        with tf_utils.maybe_init_scope(self):
            free_dims = self._query_shape.rank - 2
            einsum_equation, bias_axes, output_rank = _build_proj_equation(
                free_dims, bound_dims=1, output_dims=2, common_axes=1
            )
            ...
            einsum_equation, bias_axes, output_rank = _build_proj_equation(
                self._key_shape.rank - 2, bound_dims=1, output_dims=2, common_axes=1
            )
            ...
            einsum_equation, bias_axes, output_rank = _build_proj_equation(
                self._value_shape.rank - 2, bound_dims=1, output_dims=2, common_axes=1
            )
            ...
            if self._input_steps != self._output_steps:
                self._conv_time_steps_dense1, self._conv_time_steps_dense2 = self._make_conv_time_steps_layers(
                    free_dims,
                    "conv_time_steps"
                )
    ...
    def _make_output_dense(self, free_dims, common_kwargs, name=None):
        ...
        Returns:
          Projection layer.
        """
        output_shape = [self._output_types, self._output_depth]
        einsum_equation, bias_axes, output_rank = _build_proj_equation(
            free_dims, bound_dims=3, output_dims=len(output_shape)
        )
    ...
    def _make_conv_time_steps_layers(self, free_dims, name=None):
        """Builds the output projection matrix.
        ...
        Returns:
          Projection layer.
        """
        output_shape1 = [self._output_depth, self._key_dim]
        einsum_equation1, bias_axes, output_rank = _build_proj_equation(
            free_dims, bound_dims=1, output_dims=len(output_shape1) - 1, common_axes=1, reverse=True
        )

        output_shape2 = [self._output_depth, self._output_steps]
        ...
        return core.EinsumDense(
            ...
            **self._get_common_kwargs_for_sublayer(),
            ...
            einsum_equation2,
            output_shape=_get_output_shape(output_rank - 1, output_shape2),
            bias_axes=bias_axes if self._use_bias else None,
            name=f'{name}2',
            **self._get_common_kwargs_for_sublayer(),
        )
    ...
    def call(
        ...
        query = tf.transpose(query, _get_transpose_axes(query.shape.rank))

        if value is None:
            value = query
        else:
            value = tf.transpose(value, _get_transpose_axes(value.shape.rank))
        ...
        if key is None:
            ...
            key = tf.transpose(key, _get_transpose_axes(key.shape.rank))
        ...
        attention_output = tf.transpose(attention_output, _get_transpose_axes(attention_output.shape.rank))
